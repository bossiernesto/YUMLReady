...
class YUMLVisitor(object):
    ...
    def materialize_diagram(self, diagram):
        from YUMLConverter.YUMLDiagram import YUMLDiagram, YUMLReadyException
    ...
    def build_custom_connector(self, connector):
        return '{0}{1}{2}'.format(self.define_class(connector.fromObject), connector.association,
                                  self.define_class(connector.ToObject))
